...
write_share(){
 mkdir -p "$N4_DIR/shares"; chmod 750 "$N4_DIR/shares"
...
main(){
  ...
  [[ -n ${ip:-} ]] || ip="127.0.0.1"
  PUBLIC_IPV4="$ip"
  if [[ ${WG_PORT:-51820} == 51820 ]]; then WG_PORT=550; fi
  WG_MTU=${WG_MTU:-1280}
...
SYS_EOF
  ...
  /usr/local/bin/menu --sync-wg >/dev/null 2>&1 || true
  ...
  echo "WireGuard: UDP/$WG_PORT • MTU $WG_MTU • Keepalive 25s"
