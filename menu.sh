#!/usr/bin/env bash
set -u
N4=/etc/n4vpn; CONF=$N4/n4.conf; DEV=$N4/device-limits
...
[[ $EUID -eq 0 ]] || { echo "Run as root"; exit 1; }; mkdir -p "$DEV"
load(){ VPN_SSH_PORT=109; WS_PORTS="80,143,442,8080"; DROPBEAR_PORTS="443"; DEVICE_LIMIT_DEFAULT=1; WS_MAX_CLIENTS=2048; WS_IDLE_TIMEOUT=180; HOST_DOMAIN="$(hostname -I|awk '{print $1}')"; SLOWDNS_ENABLED=0; NS_DOMAIN=""; [[ -f $CONF ]] && source "$CONF" || true; }
save(){ cat >"$CONF" <<EOF
...
create(){ header;echo -e "${CB}${B}CREATE ACCOUNT${N}";read -r -p "Username: " u;[[ $u =~ ^[a-z_][a-z0-9_-]{2,31}$ ]]||{ echo Invalid;pause;return;};id "$u"&>/dev/null&&{ echo Exists;pause;return;};read -r -s -p "Password: " p;echo;read -r -p "Days [30]: " d;d=${d:-30};read -r -p "Device/IP limit [$DEVICE_LIMIT_DEFAULT]: " l;l=${l:-$DEVICE_LIMIT_DEFAULT};[[ $d =~ ^[0-9]+$ && $l =~ ^[0-9]+$ ]]||{ echo Invalid;pause;return;};exp=$(date -d "+$d days" +%F);useradd -M -s /bin/false -e "$exp" -p "$(openssl passwd -6 "$p")" "$u";echo "$l">"$DEV/$u";sync_limits;echo -e "${C2}Created $u • $exp${N}";echo "Host $HOST_DOMAIN | SSH $VPN_SSH_PORT | WS $WS_PORTS | Dropbear $DROPBEAR_PORTS";[[ $SLOWDNS_ENABLED -eq 1 ]]&&echo "SlowDNS $NS_DOMAIN :53";pause; }
...
deluser_n4(){ header;read -r -p "Username: " u;id "$u"&>/dev/null||{ echo Not found;pause;return;};read -r -p "Delete $u? [y/N]: " x;[[ $x =~ ^[Yy]$ ]]||return;pkill -KILL -u "$u" 2>/dev/null||true;userdel -f "$u";rm -f "$DEV/$u";sync_limits;echo Deleted;pause; }
...
domain(){ header;read -r -p "New domain [$HOST_DOMAIN]: " x;[[ -n $x ]]&&HOST_DOMAIN=$x;save;pause; }
main(){ while true;do header;echo -e " ${CB}${B}ACCOUNT${N}";echo "  [01] Create account         [02] Renew account";echo "  [03] Change password        [04] Delete account";echo "  [05] Account list           [06] Device/IP limit";echo "  [07] Live connections";echo;echo -e " ${CB}${B}SERVER${N}";echo "  [08] Port manager           [09] SlowDNS";echo "  [10] Service manager        [11] Change domain";echo "  [12] Open normal shell      [00] Exit menu";echo -e "${CP}──────────────────────────────────────────────────────────────${N}";read -r -p "Select [00-12]: " x;case $x in 1|01)create;;2|02)renew;;3|03)passwd_user;;4|04)deluser_n4;;5|05)listusers;;6|06)limit;;7|07)connections;;8|08)ports_menu;;9|09)slowdns;;10)services;;11)domain;;12)clear;echo "Type exit to return to menu";bash --noprofile --norc;;0|00)clear;return;;*)sleep .3;;esac;done; }
main "$@"
